// Convert PCM to A-law or back.
#include <common.h>

static const i16 alaw2pcm16[256] = {
...
static inline i32 alaw2linear(u8 a_val) { return alaw2pcm16[a_val]; }

static const u8 alaw_encode[2049] = {
...
static inline u8 linear2alaw(i16 pcm_val) {
    if (pcm_val >= 0) return alaw_encode[pcm_val / 16];
    else              return (0x7F & alaw_encode[pcm_val / -16]);
}
...
int main(int argc, char * argv[]) {
    ...
    if (strcmp(argv[1], "-e") == 0) {
        for (i16 pcm; fread(&pcm, sizeof(pcm), 1, stdin) == 1; ) {
            u8 alaw = linear2alaw(pcm);
            ASSERT(fwrite(&alaw, sizeof(alaw), 1, stdout) == 1, "Failed to write\n")
        }
    } else if (strcmp(argv[1], "-d") == 0) {
        for (u8 alaw; fread(&alaw, sizeof(alaw), 1, stdin) == 1; ) {
            i16 pcm = alaw2linear(alaw);
